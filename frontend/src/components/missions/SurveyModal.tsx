...
const SurveyModal: React.FC = () => {
    ...
    return (
        ...
        >
            <div className={`relative w-full max-w-lg max-h-full rounded-[2.5rem] border border-white/10 bg-gray-950 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] animate-duration-300 flex flex-col overflow-hidden ${isClosing ? 'animate-zoom-out' : 'animate-zoom-in'}`}>

                {/* Decorative Elements */}
                <div className="absolute inset-0 pointer-events-none">
                    <div className="absolute top-0 left-1/2 -translate-x-1/2 w-64 h-24 bg-blue-500/10 blur-[80px]" />
                    <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-indigo-500/10 blur-[60px]" />
                </div>
                ...
                {/* Visual Accent Bar */}
                <div className={`h-1.5 w-full shrink-0 bg-linear-to-r from-blue-500 to-indigo-500 transition-transform duration-700 origin-left ${rating === 0 ? 'scale-x-0' : 'scale-x-100'}`} />
            </div>
        </div>
    );
};
